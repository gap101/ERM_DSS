...
if __name__ == "__main__":
    ...
    #                                   start_time=start_time,
    #                                   end_time=end_time,
    #                                   experimental_lookahead_horizon=None,
    ...
    decision_coordinator = DispatchOnlyCoord(state=ts2,
    ...
    config_info = dict()
    config_info['iter_lim'] = None
    config_info['discount_factor'] = None
    config_info['uct_tradeoff'] = None
    config_info['lookahead_horizon'] = None
    ...
    config_info['exp_id'] = 'greedy_stationary_r{}_ch{}'.format(num_regions, index_of_test_chain)

    res_file_path = res_directory + 'greedy_res_' + config_info['exp_id'] + '.pkl'

    print('\n\n\n')
    print('res file: {}'.format(res_file_path))

    print('avg resp_time: ', np.mean(list(decision_coordinator.metrics['resp_times'].values())))
    print('run_id: {}'.format(config_info['exp_id']))
    print('region_count: {}'.format(num_regions))
    assert num_regions == len(region_ids)
    print('exp chain index: {}'.format(index_of_test_chain))
    print('approach: greedy')
    print('predicted chain file: {}'.format(preprocessed_chains_file_path))



    pickle.dump((config_info, decision_coordinator.metrics),
                open(res_file_path, 'wb'))
