...
from decision_making.coordinator.failure_coordinators.LowLevelCoordTest_fail import LowLevelCoord_Fail
...
if __name__ == "__main__":
    ...
    preprocessed_chains_file_path = data_directory + 'chain_data/non_stationary_chains_60_v1.pickle'

    start_time = datetime(year=2019, month=1, day=4, hour=18).timestamp()
    end_time = datetime(year=2019, month=1, day=4, hour=22).timestamp()
    ...
    valid_cell_ids, FAKE_rate_DICT = get_cell_id_and_rates(incident_df, structural_valid_cells)

    ############################################
    ### run specific
    pool_thread_count = 3  # ds machine??
    ...
    non_stationary_cell_rate_dict = pickle.load(open(data_directory + 'chain_data/non_stationary_rates.pkl', 'rb'))

    for time, rate_dict in non_stationary_cell_rate_dict.items():
        for cell in cell_ids:
            if cell not in rate_dict.keys():
                rate_dict[cell] = 0.0
    ...
    predictor = ChainPredictor_Subset_Times(preprocessed_chain_file_path=preprocessed_chains_file_path,
                                            cell_rate_dictionary=non_stationary_cell_rate_dict,
    ...
    config_info['predictor'] = 'non_stationary_predicted_chains - also use as exp incident chain (ChainPredictor_Subset_Times)'
    policy_type = ''
    config_info['exp_id'] = policy_type + '_non_stationary_WITH_FAILURE_r{}_ch{}'.format(num_regions, index_of_test_chain)
    ...
    failure_time = datetime(year=2019, month=1, day=4, hour=18, minute=2).timestamp()
    ...
    region_to_fail = sorted_regions_by_rate[0][0]
    num_resp_to_fail = 2
    ...
    pickle.dump((config_info, decision_coordinator.metrics),
                open(data_directory + 'real_non_stationary_result-' + config_info['exp_id'] + '.pkl', 'wb'))  # will overwrite
