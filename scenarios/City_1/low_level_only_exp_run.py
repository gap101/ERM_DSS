...
if __name__ == "__main__":
    ...
    end_time = datetime(year=2019, month=1, day=1, hour=2).timestamp()
    ...
    state_generator = StateGenerator(num_rows=num_rows, num_columns=num_columns, structuraly_valid_cells=structural_valid_cells)
    ...
    exp_incident_events = state_generator.get_expieremental_incident_events(incident_df,
                                                                            start_time,
                                                                            end_time,
                                                                            mean_clearance_time=mean_service_time,
                                                                            valid_cells=cell_ids)

    real_num_incidents = len(exp_incident_events)

    avg_real_incident_diffs = np.mean(np.diff([_.time for _ in exp_incident_events]))
    ...
    ##################################################
    # normal chain predictor
    # predictor = ChainPredictor(preprocessed_chain_file_path=preprocessed_chains_file_path,
    #                            cell_rate_dictionary=processed_rate_dict,
    #                            max_time_diff_on_lookup=31 * 60,
    #                            check_if_time_diff_constraint_violoated=True)
    #
    # config_info['predictor'] = 'normal_chain'
    ###################################################
    # use one of the predicted chains
    # predictor = ChainPredictorReturnOne(preprocessed_chain_file_path=preprocessed_chains_file_path,
    #                                     cell_rate_dictionary=processed_rate_dict,
    #                                     max_time_diff_on_lookup=31 * 60,
    #                                     check_if_time_diff_constraint_violoated=True,
    ...
    #                                     end_time=end_time)
    #
    # exp_incident_events  = copy.deepcopy(predictor.plus_one_chain) # TODO | remove overwrite
    #
    # exp_incident_events = [_ for _ in exp_incident_events if _.time < end_time]
    #
    # num_leave_one_out_chain_incidents = len(exp_incident_events)
    #
    # config_info['predictor'] = 'use_predicted_chain'
    ####################################################
    # Use real incidents as incident chains
    # prediction_incident_events = state_generator.get_expieremental_incident_events(incident_df,
    #                                                                                start_time,
    #                                                                                end_time + (2 * lookahead_horizon_delta_t),
    #                                                                                mean_clearance_time=mean_service_time,
    #                                                                                valid_cells=cell_ids)
    # predictor = RealIncidentPredictor(real_incident_events=prediction_incident_events,
    #                                   cell_rate_dictionary=processed_rate_dict)
    # config_info['predictor'] = 'use real incidents'
    #####################################################
    # use chains sampled from real incident distribution and one sampled chain
    # predictor = ChainPredictorAppenderReturnOne(preprocessed_chain_file_path=preprocessed_chains_file_path,
    #                                             cell_rate_dictionary=processed_rate_dict,
    #                                             start_time=start_time,
    #                                             end_time=end_time,
    #                                             experimental_lookahead_horizon=lookahead_horizon_delta_t,
    #                                             chain_lookahead_horizon=30 * 60,  # 30 minutes
    #                                             check_if_time_diff_constraint_violoated=False,
    #                                             total_chains=5,
    #                                             index_of_leave_one_out=4)
    # exp_incident_events = copy.deepcopy(predictor.plus_one_chain)  # TODO | remove overwrite
    #
    # exp_incident_events = [_ for _ in exp_incident_events if _.time < end_time]
    ...
    # num_leave_one_out_chain_incidents = len(exp_incident_events)
    #
    # config_info['predictor'] = 'use_poisson_predicted_chain'
    #######################################################
    # use predicted incident chain as the chain, to ensure everything is working
    # predictor_extract_chain = ChainPredictor_Subset(preprocessed_chain_file_path=preprocessed_chains_file_path,
    #                                                 cell_rate_dictionary=processed_rate_dict,
    #                                                 start_time=start_time,
    ...
    #                                                 chain_lookahead_horizon=30 * 60,  # 30 minutes
    #                                                 check_if_time_diff_constraint_violoated=False,
    #                                                 index_of_exp_chain=4,
    #                                                 chain_indexes_to_use_for_expierements=[0, 1, 2, 3])
    # exp_incident_events = copy.deepcopy(predictor_extract_chain.exp_chain)  # TODO | remove overwrite
    #
    # predictor = RealIncidentPredictor(real_incident_events=copy.deepcopy(exp_incident_events),
    #                                   cell_rate_dictionary=processed_rate_dict)
    #
    # exp_incident_events = [_ for _ in exp_incident_events if _.time < end_time]
    #
    # config_info['predictor'] = 'use_poisson_predicted_chain - use exp incident chain'
    #######################################################
    # experiements with additional chains and multiple test chains
    ...
    # exp_6:
    # index_of_test_chain = 55
    # exp_7:
    # index_of_test_chain = 56
    # exp_8:
    # index_of_test_chain = 57
    # exp_9:
    # index_of_test_chain = 58
    # exp_10:
    # index_of_test_chain = 59
    ...
    config_info['predictor'] = 'use_poisson_predicted_chains - also use as exp incident chain'
    #######################################################
    ...
    config_info['exp_id'] = '60_chain_-_test_index_{}'.format(index_of_test_chain)
    pickle.dump((config_info, decision_coordinator.metrics), open(data_directory + 'result-' + config_info['exp_id'] + '.pkl', 'wb'))  # will overwrite
