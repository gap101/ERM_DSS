...
if __name__ == "__main__":
    ...
    end_time = datetime(year=2019, month=1, day=3).timestamp()
    ...
    ############################################
    ### run specific
    pool_thread_count = 7  # ds machine??
    ...
    policy_type = 'll_only'
    config_info['exp_id'] = policy_type + '_non_stationary_r{}_ch{}'.format(num_regions, index_of_test_chain)
    ############################################
    # get depots
    ...
    file_name_for_experiement = 'real_non_stationary_result-' + config_info['exp_id'] + '_sttime_{}'.format(start_time) + '.pkl'
    ...
    decision_coordinator = LowLevelCoord(environment_model=env_model,
                                         ...
                                         periodic_dump_file=data_directory + '_upd_' + file_name_for_experiement)
    #########################################
    ...
    pickle.dump((config_info, decision_coordinator.metrics),
                open(data_directory + 'real_nsr-' + config_info['exp_id'] + '_sttime_{}'.format(start_time) + '.pkl', 'wb'))  # will overwrite
